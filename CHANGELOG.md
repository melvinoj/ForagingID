...
Closed — 11/11 recovered from syncthing_ingest logs with conf 0.9 matching each row's stored plant_detect_confidence exactly, tying log to row rather than neighbour. Band-only diff, edited_by='system:kingdom_gate_band_repair' rather than 'human' — correctly refusing to plant a false human lock while a machine corrects a machine's write.
Current State block for the End Session box at http://127.0.0.1:8000/settings:

Current State
This session:
Review card below_threshold render gap fixed. _makeCardHtml's branch chain at review.html:1809 had no arm for below_threshold — speciesHtml stayed '' and interpolated as nothing. Silent empty string, no throw; server log, console and network tab all clean because the code did exactly what it was written to do, which was nothing. Not an exception, structurally impossible: one atomic innerHTML = observations.map(_makeCardHtml).join('') means a throw kills all 25 cards, not 4. #22089 escaped only because a manual correction flipped it to identified, which has a branch. Fixed; cards now render an honest state and an edit target. "Edits don't sustain" was never a persistence bug — there was no edit target to save into. Copy corrected twice: my first wording ("No result above the confidence threshold") invented a cause; rows had species_candidates_json=[] from transport failure, no candidates existed to filter. Now keys off the stored candidate list — candidates present → "none above threshold"; empty → "no candidates on record".
No regression in candidate display. 20 versions back to 6 June: below_threshold present in zero, sp-alts always inside the identified branch, and even that only ever showed top+2 names without scores. The full near-0% list lives in Second Opinion / Request ID — live API calls, up to 8 per provider. Those panels came back empty on 15 July because of the DNS failure, not a display change.
⚠️ iNat/PlantNet failure messaging — the code knew and threw it away. inaturalist.py:195 classifies eight states (ok, ok_empty, token_expired, rate_limited, error, unreachable, file_error, no_token), records them, returns [] for all. reidentify.py saw [] and guessed: "API error or rate limit or expired token". reidentify.py:511's bare except Exception: return None destroyed PlantNetError.is_connection_error. last_inat_status() was already exposed at /api/me, already consumed by scan.py:1079 — reidentify never called it. 15 July failures were DNS ([Errno 8] nodename nor servname provided), recorded verbatim in processing_logs as state=unreachable the whole time the card said "expired token". Token was valid throughout, 6/6 live calls. Fixed: _inat_warning()/_plantnet_warning() read recorded state, is_connection_error preserved, inat_state/plantnet_failed added to retry-identify response. Blast radius narrower than feared — identification.py:150/168 was already the reference implementation, scan.py/sharing.py correct; defect contained to reidentify.py, which never adopted a pattern one directory over. Proved itself live: iNat token genuinely expired mid-task (HTTP 401), new code named the real fault and stayed silent about PlantNet, which was returning 79.16%.
PlantNet transport — 3,514 write timeouts since 3 June, ~1-in-8, seven weeks, not a regression. Not bandwidth: 4MB at 8.99 Mbit/s needs ~3.6s; successes cluster 3.22–5.02s, failures blow past 8s. requests timeout is per-socket-operation, not total — "8s" never meant "upload may take 8s". Momentary stalls on a link shared with ngrok and Syncthing. Fixed: REQUEST_TIMEOUT_S = 8 → (5, 25) (connect=5 preserves the fail-fast that mattered for the DNS failure), plus bounded retry — 3 attempts, 0.5s then 1.5s backoff, ~1-in-512 residual. Retries only on requests.Timeout/ConnectionError, never an HTTP status (a 429 makes exactly one POST). attempts logged only when >1 so the rate stays measurable. httpx swap considered and rejected — the fault isn't the library; parked as backlog. 22099 verified live: dual agreement 80.39/87.83 Circaea lutetiana. retry-identify is read-only by contract, does not auto-approve.
Triage restored and persisted. 15 July's keeper calls existed only in a session transcript — zero of 1,271 rows carried any mark, no column existed to hold one. Recovered from past-conversation search. Alembic 0050: triage_keep BOOLEAN nullable NULL (three-state: NULL untriaged / 1 keeper / 0 explicit discard) + triage_keep_at. review_label correctly rejected as mechanism — machine-written pipeline output, rescan-overwritable, live query filter. 141/141 marked, zero dupes. Sheet size resolved as 60 from the printed sheet header (1,014 divides cleanly by both 60 and 63 — arithmetic could not distinguish, and the two give completely different sets). Method self-validated: Code's derived offset-780 slice matched the sheet-read exactly, including the non-monotonic 20509/20512/20510/20511 run.
never_reject enforced, not prose. BOOLEAN column, veto inside delete_observation_file() at the destructive call site — all reject paths funnel through it. 21212/21215/21216 set (thumbnail is the only surviving copy). Behaviourally tested.
⚠️ Ingest blacklist gate — five paths, one had the check. deleted_hashes was consulted only by syncthing.py (P1). p2_scan_image, p2_upload, folder_scan, and p2_archive_scan had nothing — and p2_archive_scan is the DIGIERA rescan, the exact event the blacklist exists to prevent. Since DELETE removes the row, dedupe on Observation.file_hash couldn't catch it either. Never existed in any of 21 recoverable versions — birth defect, not drift; docstring claimed otherwise. Fixed via shared app/services/ingest_guard.py, five call sites. Chokepoint deliberately not built — would mean restructuring ingest, a sixth path could still miss it; BEFORE INSERT trigger rejected (opaque exception mid-transaction, load-bearing logic outside the app). ingestion.py's else: failed += 1 was counting blacklist skips as failures — fixed. 14/14 behavioural, skip-log counter 1→5 proving each path logs independently. deleted_hashes is permanent: no removal path, no endpoint. Blacklisting a hash forecloses that photo from every source, forever.
1,113 rejected. Predicate-derived, not transcribed. Every exclusion held: 141/141 keepers intact, 21212/21215/21216 thumbnails intact, 5 private DELETEs untouched, deleted_hashes 12. Undo dir drained to zero — files genuinely gone, verified by ID. Integrity exact against db_20260717_090803.sqlite. Blanket blacklist correctly dropped: reject keeps the row and file_hash, so dedupe already blocks re-ingest; blacklist is only needed where DELETE removes the row.
My predicate said 1,122 — nine were never triaged. pending_identification, not not_plant, so never in the triage universe: the 25 sheets were generated from not_plant only (1014+50+44+19+15+5+1 = exactly 1,262). The 9 are CHANGELOG's long-flagged 0.7%. Six aren't on DIGIERA — 2026 phone photos post-consolidation (DIGIERA is the P2 archive, not a P1 backup). The "stop if not exactly 1,122" gate held only because Code investigated the mismatch rather than reconciling it.
⚠️ _identify_scanned 104-line unprotected prologue. Function starts at 938; its try: opens at 1042. Anything thrown in between — including AsyncSessionLocal() blocking on SQLite lock contention — escaped the handler at 1425 that would have written stage='identify', status='failed'. Row stays ingested: no log, no status change, no error. Exact signature of all 9. Failure was durable nowhere: P1's except (syncthing.py:777) increments in-memory counters lost on restart; P2's background_tasks.add_task has the same hole. Not an interrupted run — stalls interleaved with successes, 11× on 2026-06-07, 5× on 2026-06-14, up to 4 rows created in the same second. Ceiling exactly 9, no hidden population. Fixed: prologue guard (_identify_scanned now a wrapper around _identify_scanned_inner), _mark_identify_failed() writes a durable processing_logs row, and app/services/orphan_sweep.py — every 10 min, pipeline_try_acquire non-blocking so it never competes with a live scan, 15-min grace period (a row at ingested for the duration of its own identification is excluded), cap 20/cycle, re-queues with force_review=True, routing unchanged, logs what it sweeps. 10/12 pass; 2 failures were the kingdom gate, caught exactly as the test was designed to.
⚠️⚠️ iNat kingdom gate auto-rejected and deleted on a probabilistic guess. scan.py:1250 and a twin at identification.py:189 rejected + called _delete_file() when iNat's top candidate wasn't in {plantae, fungi} at ≥5%, ignoring force_review. Fired on 4 of the 9 and was wrong all four: 20223 (dog 6.8%), 20560 (Eriophyes tiliae 48.4% — a lime gall mite named instead of the plant it's on), 21178 (cat 12.1%), 21250 (cow 9.4%). Two survive only because Code's test script restored them from /tmp/foragingid_undo before the 30s timers fired. Docstring claimed ≥5% makes the subject "definitively" non-plant — false. Violates the doctrine directly: auto-approve requires dual agreement, auto-reject required nothing. Both paths fixed — route to needs_review, review_label='non_plant', no delete call, force_review honoured trivially (the branch has no reject path left), reason recorded as signal not verdict. 12/12 behavioural. 4/4 recovered (20223 from DIGIERA, hash-verified).
Kingdom-gate historical audit: 97 rejections since 2026-06-06, 93 still rejected, zero permanently gone (82 DIGIERA, 11 Syncthing source). Median 22.2%, min 5.1%, 29 of 97 fired under 10%. Two populations: 13 Insecta — every one a plant-dwelling organism (gall wasps, leaf miner, ash aphid, viburnum beetle, #21965 Bombus frigidus 51.1%), the model naming the organism on the plant; and 71 Mammalia (Code corrected its own "73" — log rows, not observations; 19447 had three duplicate gate logs).
13 Insecta restored and routed to review. 11 Syncthing + 2 DIGIERA — every Syncthing-source kingdom-gate row is Insecta. 13/13 sha256-verified, thumbnails regenerated, needs_review, no identification re-run, deleted_hashes untouched at 12. Exactly 13 rows changed DB-wide. These had never appeared on any surface: rejected rows don't reach review, and they weren't not_plant so weren't on the contact sheets either.
Gate-clobbered prefilter band repaired on 11. The gate overwrote prefilter_category='person_animal' on rows the prefilter had passed as plant at conf 0.900. Original verdict recovered from each row's own syncthing_ingest log (prefilter=plant conf=0.900), with log conf matching stored plant_detect_confidence exactly — tying log to row, 11/11 recovered, 0 guessed. 18180 (conf 0.117) and 19361 (conf 0.05) left alone — they genuinely failed the prefilter. edited_by='system:kingdom_gate_band_repair', deliberately not 'human' — a machine correcting a machine's false write must not plant a human lock. Band-only diff proven: 11 rows prefilter_category, 0 rows status/obs_category.
Pending / next:
P1 intent as a routing veto — top of the list. Syncthing/P1 photos are ones Melvin chose to send; P2 archive rows are a bulk drive sweep. The pipeline treats both identically at every gate. is_phone was built to mean "manual upload → always review" and the fungi rule sits in the same place — the doctrine exists, it just isn't wired into reject paths at all. Live instance: identification.py:324 (below).
⚠️ identification.py:324 — "no candidates → auto-reject" also deletes and also ignores force_review, and fires on transport failure (pn_error set). P1 syncthing does the opposite and routes to review with the comment "P1 (syncthing) must never be auto-rejected on confidence." Same photo, different fate, decided by which folder it arrived in. Very likely the mechanism behind the 2,083.
2,083 rejected rows had zero candidates AND a PlantNet transport failure — rejected on evidence the pipeline never gathered. Recoverable from DIGIERA/phone only. Separate decision, same shape as the Mammalia.
68 Mammalia + 5 Aves + 2 Animalia + 1 Chromista + 1 Reptilia + 1 Mollusca still kingdom-gate rejected — all recoverable. 29 of 97 fired under 10% confidence. Melvin's call.
141 keepers → review via override-prefilter — chokepoint safe since the 15 July force_review fix.
7 DELETEs with hash blacklist — 13623, 13368, 20066, 20053, 20022 (pending) + 19436, 19437 (already rejected). Passport pages ×2 and private child photos. Ingest gate now real on all five paths. Permanent foreclosure — final.
The 9 stalled rows are in review — 9/9 needs_review, files and thumbnails on disk.
Fail-quiet is the defect class of this session — missing else, _dual_agree, guessed API messages, P2 blacklist gap, unprotected prologue, both kingdom gates, else: failed += 1. Same shape every time: the rule exists, the veto doesn't fire, the docstring claims otherwise. A branch-chain audit across review.html and the reject paths is the pass that closes the class rather than the instances.
Retry ID / Request ID buttons render inconsistently across review card states — absent on below_threshold cards, where retry is most useful. Suspected same missing-branch class. Not diagnosed.
Dark theme / Option B tokens.css — scope decided: token structure built to support multiple themes, only the current dark palette shipped. No switcher. :root holds names, dark values in a themeable block, per-page inline literals deleted in favour of var(). Adding a theme later becomes one block, not 11 copy/pastes. Print styles (WeasyPrint, inlined CSS) stay independent — correct.
Fix pass sequence agreed: shared render helper for safety text first, then map popup excerpts + print/lists truncation land through it, then review history table reflow separately (layout problem, not truncation).
P1/P2 concurrency mutex — arrived as data this session: SQLite lock contention is what stalled the 9.
Parked:
httpx swap for plantnet.py — one HTTP library across both integrations; nothing retry doesn't already deliver. Post-October.
[iNaturalist] id_notes are encyclopedia prose, not field-ID text.
Truncation still live on review history table, map/encounters line-clamps, recipe previews.
